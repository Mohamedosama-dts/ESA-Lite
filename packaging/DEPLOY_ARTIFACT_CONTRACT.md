...
|----------------|-------------|
| `ESA_Lite_v2.1.0.exe` | **ESA-Lite-Core-Executable** |
| `ESA_Lite_en.msi` | **ESA-Lite-Localized-MSIs** (EN) |
| `ESA_Lite_ar.msi` | **ESA-Lite-Localized-MSIs** (AR) |

When the product version changes, replace `2.1.0` in the EXE name to match `AppConfig.AGENT_VERSION` / the WiX `Product/@Version`. MSI **base** names stay `ESA_Lite_en.msi` / `ESA_Lite_ar.msi`.

CI still uploads Actions artifacts on `main` (7-day retention) for debugging. **They are not the production feed.**

## Consumer obligations

1. Pin an explicit tag (`v2.0.0`, `v2.1.0`, …).
...
- Burn bundle `ESA_Lite_Setup_v2.1.0.exe` / artifact `ESA-Lite-Final-Setup`
- Any signing-agent / NATS / secret injection

## Example pull

```bash
gh release download v2.1.0 -R Mohamedosama-dts/ESA-Lite \
  -p ESA_Lite_en.msi \
  -p ESA_Lite_ar.msi \
  -p ESA_Lite_v2.1.0.exe \
  -D /tmp/esa-lite

test -f /tmp/esa-lite/ESA_Lite_en.msi
test -f /tmp/esa-lite/ESA_Lite_ar.msi
test -f /tmp/esa-lite/ESA_Lite_v2.1.0.exe
```
